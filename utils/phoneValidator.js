...
export function normalizeMexicanPhoneNumber(phoneNumber) {
  ...
  if (hasPlusPrefix1) {
    ...
    if (cleanNumber.length !== 14 || cleanNumber.length !== 13) {
      logger.error(
        `Número con formato +${NUMBER_PREFIX}1 debe tener 14 dígitos en total`
      );
      return null;
    }
    return cleanNumber;
    ...
    if (cleanNumber.length !== 13) {
      logger.error(
        `Número con formato +${NUMBER_PREFIX} debe tener 13 dígitos en total`
      );
      return null;
    }
    return cleanNumber;
    ...
    if (digitsOnly.length === 10) {
      ...
      digitsOnly.startsWith(`${NUMBER_PREFIX}`)
    ) {
      // Número con 52 pero sin el +
      return `+${digitsOnly}`;
    } else if (
      digitsOnly.length === 13 &&
      digitsOnly.startsWith(`${NUMBER_PREFIX}1`)
    ) {
      // Número con 521 pero sin el +
      return `+${NUMBER_PREFIX}${digitsOnly}`;
    } else {
      logger.error(
        `Formato de número inválido: ${phoneNumber}. Se espera un número de 10 dígitos (local) o con prefijo +${NUMBER_PREFIX}/+${NUMBER_PREFIX}1`
      );
      return null;
    }
  }
