      ...
      if (existingNumber) {
        ...
        phoneNumbersToCreate[i] = {
          ...
          hasReceivedVerificationMessage:
            existingNumber.hasReceivedVerificationMessage || false,
        };
      }
    }
  ...
  try {
    ...
    const validPhoneNumbers = await db.PhoneNumbers.findAll({
      where: {
        ...
        [Op.or]: [{ status: "por verificar" }, { status: "no verificado" }],
      },
      transaction: t,
      raw: true,
    });
    ...
    for (let i = 0; i < validPhoneNumbers.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          logger.info(
            `Mensaje enviado con SID: ${
              messageResult.sid ? messageResult.sid : "No se envio"
            }`
          );
          ...
          try {
            ...
            logger.error(
              "Error al hacer rollback o reiniciar transacción:",
              rollbackError
            );
      ...
      const batchStats = `Lote de ${
        batch.length
      } números: ${successCount} exitosos, ${errorCount} fallidos en ${batchDuration.toFixed(
        2
      )}s`;
      ...
      include: [
        {
          ...
          as: "messages",
          attributes: {
            exclude: ["twilioSid", "phoneNumberId", "createdAt"],
          },
          include: [
            {
