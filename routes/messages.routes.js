...
router.post("/", async (req, res) => {
	...
	if (!phoneNumbers || !Array.isArray(phoneNumbers)) {
		return res.status(400).json({
			error: "phoneNumbers debe ser un array válido",
		});
	}
	...
	try {
			...
			phoneNumbers.map(async (phoneNumber) => {
				...
				if (!phoneRecord) {
					results.push({
						phoneNumber,
						status: "error",
						error: `Número no encontrado en la BD.`,
					});
		...
		await t.rollback();
		console.error(
			"Error en la transacción de envío de mensajes:",
			dbError
		);
		res.status(500).json({
...
router.post("/response", async (req, res) => {
	...
	if (MessageType === "text") {
		...
		return res
			.status(200)
			.json({ message: "Respuesta no es un boton" });
	}
	...
	try {
		...
		if (!respondedMessage) {
			await t.rollback(); // Revertir por seguridad
			logger.error(
				`Mensaje no encontrado: ${OriginalRepliedMessageSid}`
			);
			logger.info("-------------------------------------");
			return res.status(404).json({
				error: `Mensaje no encontrado: ${OriginalRepliedMessageSid}`,
			});
		}

		// actualiza el mensaje
		logger.info(
			`Actualizando el mensaje: ${JSON.stringify(
				respondedMessage,
				null,
				2
			)}`
    ...
    logger.info(`MessageSid: ${MessageSid}`);
		...
		for (let i = 0; i < MAX_RETRIES; i++) {
			try {
				...
				if (messageRecord) {
					logger.info(
						`Mensaje ${MessageSid} encontrado en el intento ${
							i + 1
						}`
					);
					break;
				...
				if (i === MAX_RETRIES - 1) {
					logger.error(
						`Mensaje no encontrado después de ${MAX_RETRIES} intentos`
					);
					await t.rollback();
		...
		if (messageRecord) {
			...
			const currentPrecedence =
				STATUS_PRECEDENCE[currentStatus] || 0;
			...
			if (newPrecedence > currentPrecedence) {
				...
				logger.info(
					`✅ Estado del mensaje actualizado a ${newStatus}`
				);
				logger.info(
					"||||||||||||||||||||||||||||||||||||||||"
				);
				...
				if (
					newStatus === "delivered" ||
					newStatus === "read" || newStatus === "undelivered"
				) {
					const phoneNumberString = To.replace(
						"whatsapp:",
						""
					);
					const numbersToSearch =
						getSearchablePhoneNumbers(
							phoneNumberString
						);
					logger.info(
						"numbersToSearch: ",
					...
					if (!phoneRecord) {
						...
						logger.error(
							`Número de destino no encontrado o su estado ya es verificado: ${To}`
						);
						logger.info(
							"----------------------------------------"
						);
						...
						await phoneRecord.update(
							{
								status: newStatus === "undelivered" ? "no verificado" : "verificado",
								updatedAt: new Date(),
							},
							{ transaction: t }
						);
						...
						logger.info(
							`✅ Número de destino actualizado a verificado: ${To}`
						);
						logger.info(
							"||||||||||||||||||||||||||||||||||||||||"
						);
				...
				logger.info(
					`Estado no actualizado: precedencia menor o igual`
				);
				logger.info(
					"||||||||||||||||||||||||||||||||||||||||"
				);
